...
class InduRebalanceTuesdayStrategy(bt.Strategy):

    params = dict(
        selected_industries={},  # 新增：允许从外部传入行业候选列表， key 日期， value 为对应行业列表
        indu_type='A',
        stock_industry_map={},  # <== 新增参数，股票代码 → 行业名
    ...
    def filter_stocks(self):
        ...
        # 获取指定策略类型的行业列表
        selected_industries = []
        if hasattr(self.p, 'selected_industries') and self.p.selected_industries:
            indu_map = self.p.selected_industries.get(self.p.indu_type, {})
            selected_industries = indu_map.get(dt.strftime('%Y-%m-%d'), [])
            if selected_industries:
                print(f"✅ 当前日期 {dt} 使用策略 {self.p.indu_type} 筛选行业，行业列表: {selected_industries}")
            else:
                print(f"⚠️ 当前日期 {dt} 策略 {self.p.indu_type} 未找到对应行业列表，跳过行业过滤")
        else:
            print("ℹ️ 未启用行业轮动过滤")
        ...
        for d in self.datas:
            ...
            try:
                ...
                stock_code = d._name[-6:]  # 提取 000002 用于匹配
                stock_industry = self.p.stock_industry_map.get(stock_code, None)
                # 📌 过滤行业
                if selected_industries and stock_industry not in selected_industries:
                    print(f"⚠️ 当前股票 {d._name} 跳过行业 {stock_industry}")
                    continue
