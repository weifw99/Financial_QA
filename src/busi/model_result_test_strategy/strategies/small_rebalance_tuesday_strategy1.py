...
class SmallRebalanceTuesdayStrategy1(bt.Strategy):
    ...
    def next_open(self):
        ...
        if weekday == self.p.rebalance_weekday and self.rebalance_date != dt.date():
            ...
            for d in self.to_sell_list:
                ...
                self.close(d) # 卖出 , 在 next open 中卖出，价格是 open 价
                self.to_sell_list = []
        ...
        if self.rebalance_flag and self.to_buy_list:
            ...
            total_per_stock = total_value * 0.99 / max(len(self.to_hold), 1)
    ...
    def check_individual_stop(self):
        for data in self.datas:
            ...
            buy_price = pos.price
            current_price = data.open[0] # next open 判定要使用 open 价，使用close 会存在数据泄露
    ...
    def check_individual_stop1(self):
        for data in self.datas:
            pos = self.getposition(data)
            if pos.size <= 0:
                continue
