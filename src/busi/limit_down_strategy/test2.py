# filename: trend_emotion_timing_backtest_local.py
"""
复现 Reiss (2025) Trend-Emotion-Timing 策略
版本：使用本地/自有数据源（用户提供的 stocks 数组结构）
输入: stocks = [{'code': code, 'data': df}, ...]
每个 df 至少包含 ['date','open','high','low','close']
"""
import os
import logging
...
# -------- Trend / Emotion / Anchored 计算 --------
def compute_trend_score(df):
    close = df['close']
    roc_periods = [24,32,48,64,96,128,192,256,384,512]
    sma_periods = roc_periods
    crossover_pairs = [(20,50),(20,100),(20,200),(20,400),(50,100),(50,200),(50,400),(100,200),(100,400),(200,400)]
    lr_months = [3,4,5,6,7,8,9,12,15,18]
    lr_days = [int(m*21) for m in lr_months]
    votes = []
    for n in roc_periods:
        v = np.sign(close - close.shift(n)).fillna(0); votes.append(v)
    for n in sma_periods:
        s = sma(close, n); v = np.sign(close - s).fillna(0); votes.append(v)
    for s_short, s_long in crossover_pairs:
        s1, s2 = sma(close, s_short), sma(close, s_long)
        v = np.sign(s1 - s2).fillna(0); votes.append(v)
    for w in lr_days:
        slope = linear_regression_slope(close, w)
        v = slope.apply(lambda x: 1 if x>1e-8 else (-1 if x<-1e-8 else 0)).fillna(0)
        votes.append(v)
    votes_df = pd.concat(votes, axis=1)
    return votes_df.mean(axis=1)
...
def compute_emotion_index(df):
    ...
    for n in rsi_periods:
        r = rsi(close, n)
        osc.append((2.0*(r/100.0)-1.0).rename(f"RSI_{n}"))
    for n in cr_periods:
        cr = candle_range_index(close, high, low, n)
        osc.append(cr.rename(f"CR_{n}"))
    return pd.concat(osc, axis=1).mean(axis=1)
...
def process_single_stock(code, df, index_close=None):
    try:
        ...
        if index_close is not None:
            ratio_close = df['close'] / index_close.reindex(df.index).fillna(method='ffill')
            ratio_df = pd.DataFrame({
                'close': ratio_close,
                'high': df['high']/index_close.reindex(df.index).fillna(method='ffill'),
                'low': df['low']/index_close.reindex(df.index).fillna(method='ffill')
            })
            ratio_trend = compute_trend_score(ratio_df)
        ...
        emotion = compute_emotion_index(df)
        ...
        logging.info(f"✅ {code} 处理完成，数据点={len(out)}")
...
def backtest(stocks_processed, rebalance_days=REBALANCE_DAYS, top_n=TOP_N):
    ...
    for i in range(0, len(all_dates), rebalance_days):
        date = all_dates[i]
        if date not in timing_df.index: continue
...
if __name__ == "__main__":
    ...
    results = []
    for s in stocks[:50]:  # 示例只处理前50支，避免太慢
