...
适配中国A股，使用用户自有数据结构：
    stocks = [{'code': code, 'data': df}, ...]
每个 df 至少包含 ['date','open','high','low','close']

功能：
1. 计算每只股票的全部指标（RoC/SMA/Crossover/LR/RSI/CR）
2. 保存单股指标文件 results/{code}_TET_full.csv
3. 执行组合回测，输出净值曲线、持仓矩阵、汇总文件
4. 自动记录日志 trend_emotion_log_*.log
"""
...
SLIPPAGE = 0.0005
SAVE_FULL_DETAIL = True     # 是否保存全部指标列
MAX_STOCKS = 50             # 限制前N支股票测试（避免测试太慢）
...
def compute_trend_score(df):
    ...
    for n in roc_periods:
        components[f"RoC_{n}"] = np.sign(close - close.shift(n)).fillna(0)
    for n in sma_periods:
        s = sma(close, n)
        components[f"SMA_{n}"] = np.sign(close - s).fillna(0)
    for (s_short, s_long) in crossover_pairs:
        s1, s2 = sma(close, s_short), sma(close, s_long)
        components[f"X_{s_short}_{s_long}"] = np.sign(s1 - s2).fillna(0)
    for w in lr_days:
        slope = linear_regression_slope(close, w)
        components[f"LR_{w}"] = slope.apply(lambda x: 1 if x>1e-8 else (-1 if x<-1e-8 else 0)).fillna(0)
...
if __name__ == "__main__":
    ...
    for stock in stocks_raw:
        if stock["code"] == index_code:
            index_series = stock["data"].set_index('date1')['close']
        else:
            stocks.append(stock)
