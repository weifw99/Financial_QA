...
class CsiSmallRebalanceTuesdayStrategy(bt.Strategy):
    ...
    def filter_stocks(self):
        ...
        for d in self.datas:
            try:
                ...
                if (lt_mv > self.p.min_mv
                    ...
                    candidates.append((d, mv, score, source_))
            except:
        ...
        for d, mv, score, source_ in candidates:
            if source_ not in candidates_by_source:
                candidates_by_source[source_] = []
            candidates_by_source[source_].append((d, mv, score, source_))

        # 对每个分组内部按 score 降序排序，并取top100
        sorted_candidates = []
        for source_value in sorted(candidates_by_source.keys()):  # 按 source_ 排序
            group = candidates_by_source[source_value]
            group_sorted = sorted(group, key=lambda x: x[2], reverse=True)
            # 每组取 top 100
            top_from_group = group_sorted[:100]

            # 按照市值升序排序
            candidates1 = sorted(top_from_group, key=lambda x: x[1], reverse=False)
            sorted_candidates.extend( candidates1[:int(self.p.hold_count_high / 2)] )

        # 最终取前 self.p.hold_count_high 只股票
        final_candidates = sorted_candidates[:self.p.hold_count_high]

        print('candidates:', final_candidates)
        if len(final_candidates) > 0:
            print("filter_stocks len：", len(final_candidates), f'{final_candidates[0][0]._name} mv min: ',
                  final_candidates[0][1],
                  f'{final_candidates[-1][0]._name} mv max: ', final_candidates[-1][1])
        else:
            print("filter_stocks len：", len(final_candidates))

        return [x[0] for x in final_candidates]
