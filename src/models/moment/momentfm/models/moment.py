...
from transformers import T5Config, T5EncoderModel, T5Model, AutoConfig

from models.moment.momentfm.common import TASKS
from models.moment.momentfm.data.base import TimeseriesOutputs
from models.moment.momentfm.models.layers.embed import PatchEmbedding, Patching
from models.moment.momentfm.models.layers.revin import RevIN
from models.moment.momentfm.utils.masking import Masking
from models.moment.momentfm.utils.utils import (
...
class MOMENT(nn.Module):
    def __init__(self, config: Namespace | dict, **kwargs: dict):
        ...
        self.freeze_encoder_layers: list[str] = [f'block.{i}.' for i in config.getattr("freeze_encoder_layers", [])]
        ...
        if self.freeze_encoder:
            self.encoder = freeze_encoder_parameters(self.encoder, self.freeze_encoder_layers)
        if self.freeze_head:
...
class MOMENTPipeline(MOMENT, PyTorchModelHubMixin):
    def __init__(self, config: Namespace | dict, model_type: str='pre_model', **kwargs: dict):
        """
        :param config:
        :param model_type: 加载模型的来源，取值pre_model、ft_model; pre_model:开源模型：如MOMENT-1-small，ft_model：基于开原模型微调之后的模型
        :param kwargs:
        """
        self._validate_model_kwargs(**kwargs)
        kwargs_temp = kwargs.get("model_kwargs", {}).copy()
        if 'pre_model' == model_type:
            # 加载原始模型需要使用 reconstruction初始化
        ...
        self.config.model_kwargs = kwargs_temp
    ...
    def init(self) -> MOMENT:
        if self.new_task_name != TASKS.RECONSTRUCTION:
            self.task_name = self.new_task_name
            self.head = self._get_head(self.new_task_name)

            print(self.config)
            # head_model_path = os.path.join(self.config.model_name_or_path, "head_model_path.pt")
            # if os.path.exists(head_model_path):
            #     logging.etf_info("Loading head model from {}".format(head_model_path))
            #     self.head.load_state_dict(torch.load(head_model_path))
        return self
...
def freeze_encoder_parameters(model, t5_block_indexs:list[str]=[]):
    """
    Freeze parameters of the model
    """
    if len(t5_block_indexs) > 0: # Freeze lay the parameters
        static_names = [
            "embed_tokens.weight",
            "embed_tokens.bias",]
        for name, param in model.named_parameters():
            if name in static_names:
                param.requires_grad = False
                continue
            for t5_block_index in t5_block_indexs:
                if name.startswith(t5_block_index):
                    param.requires_grad = False
    else:
        return freeze_parameters(model)
    return model
