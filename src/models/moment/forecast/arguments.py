...
class ModelArguments(TrainingArguments):
    ...
    freeze_encoder_layers: list[str] = field(
        default_factory=list,
        metadata={"help": "冻结的编码器层"}
    )
...
class DataArguments:
    ...
    data_type: str = field(
        metadata={"help": "数据类型：min60, daily, weekly"}
    )
    ...
    train_data_stride_len: int = field(
        default=1,
        metadata={"help": "训练数据步长"}
    )
    test_data_stride_len: int = field(
        default=1,
        metadata={"help": "测试评估数据步长"}
    )
